...
def test_process_exits_standard_exit_should_succeed(testlang, num_outputs, plasma_framework):
    ...
    exit_id = plasma_framework.getNextExit(NULL_ADDRESS_HEX)
...
def test_finalize_exits_for_erc20_should_succeed(testlang, plasma_framework, token):
    ...
    assert standard_exit == [owner.address, token.address, amount, spend_id]
    ...
    assert finalized_standard_exit == [NULL_ADDRESS_HEX, NULL_ADDRESS_HEX, 0, 0]
