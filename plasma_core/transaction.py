...
from plasma_core.utils.transactions import encode_utxo_id, owner_to_output_guard
...
class TransactionOutput(rlp.Serializable):
    fields = (
        ('output_guard', rlp.sedes.Binary.fixed_length(32)),
    ...
    def __init__(self, owner=NULL_ADDRESS, token=NULL_ADDRESS, amount=0):
        ...
        amount = amount
        super().__init__(owner_to_output_guard(owner), token, amount)
...
class Transaction(rlp.Serializable):
    ...
    def __init__(self,
        ...
        """
        if inputs is None:
            inputs = [Transaction.DEFAULT_INPUT]
        if outputs is None:
            outputs = [Transaction.DEFAULT_OUTPUT]
